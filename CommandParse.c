...
#define NODE_DEBUG 0
#define ENABLE_WIDTHER_SUPPORT 0
...
void showList(void)
{
    ...
    {
        (node->isWch)
            ? wprintf(L"%u:command  <%ls>(isWch)     ", cnt++, node->command_w)
            : printf("%u:command  <%s>     ", cnt++, node->command);
#if 0
        printf("%u %u %u %u %u",
               *(node->command_w),
               *((node->command_w) + 1),
...
static void *ParseSpace(const char *userParam)
{
    ...
    char tmpArr[MAX_PARAMETER] = {0};
    if (userParam == NULL)
    {
        printf("<%s> userParam is null...\n", __func__);
        return NULL;
    }
    ...
    do
    {
        ...
        if ( userData == tmp )
        {
            printf("<%s> userParam == tmp...  userData:%u\n",
             __func__, userData->len);
            free(userData);
 ...
 the end string has no end\n",
        ...
        memset(tmpArr, 0, MAX_PARAMETER);
        memcpy(tmpArr,
               (userData + userDataCnt - 1)->strHead,
               (userData + userDataCnt - 1)->len);
#if NODE_DEBUG
        printf("<%s>parse: %s\n", __func__, tmpArr);
#endif
...
static void regParam(void *arg)
{
    ...
    if (data == NULL)
    {
        printf("<%s>userParam error, len:%u, userData:%p\n",
               __func__, paramCnt, userData);
        return;
    }
