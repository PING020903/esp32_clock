...
void showList(void)
{
    ...
    {
        (node->isWch)
            ? wprintf(L"%u:command  <%ls>(isWch)     ", cnt++, node->command_string)
            : printf("%u:command  <%s>     ", cnt++, (char*)(node->command_string));
        showParam(node);
        node = node->next;
...
#define PRINT_CMD(NODE) (isWch)\
    ...
    else
    {
        ...
        if ( repeating != NULL )
        {
            printf("<%s>The same command already exists...\n", (char*)cmdStr);
            free(CmdNode);
                      ...
                      const void* paramStr)
{
 ...
 as the command node...\n";
    ...
    else
    {
        ...
        {
            printf("<%s>The same parameter already exists...\n", (char*)paramStr);
            return lastError = NODE_REPEATING;
        }
        paramNode = (parameter_node*)malloc(sizeof(parameter_node));
        CHECK_BUF(paramNode);
...
static void* ParseSpace(const char* userParam)
{
    ...
    do
    {
        ...
        {
#if NODE_DEBUG
            printf("--<%s>%d--too far:%d, strAdd:%p, tooFarAdd:%p\n",
...
/// @brief 用于JSON格式数据的参数处理
/// @param commandString 
/// @param data 外部对象
/// @return
int CommandParse_JSON_data(const char *commandString, const User_httpd_data *data)
{
    const char *spaceStr = " ";
    command_node *CmdNode = NULL;
    parameter_node *ParamNode = NULL;
    ...
    const char *tmp = NULL, *tmp2 = NULL;
    size_t len = 0;
    if (commandString == NULL || data == NULL)
        return NODE_ARG_ERR;
