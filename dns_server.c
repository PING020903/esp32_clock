...
void start_dns_server(void)
{
    ...
    xTaskCreatePinnedToCore(dns_server_task, "dns_server", 4096, NULL, 5, &DNS_server_task_handle, 1);
}
...
esp_err_t User_DNS_server_init(void)
{
    ...
    err = mdns_hostname_set(LOCAL_HOST_NAME);
    if(err)
    {
    ...
    err = mdns_instance_name_set(LOCAL_SERVER_NAME);
    if(err)
    {
    ...
    err = mdns_service_add(LOCAL_SERVER_NAME, "_http", "_tcp", 80,
                           serviceTxtData, sizeof(serviceTxtData) / sizeof(serviceTxtData[0]));
    if(err)
    {
        ESP_LOGW(TAG, "MDNS Service add failed: %d", err);
        return err;
    }

    netbiosns_init();
    netbiosns_set_name(LOCAL_HOST_NAME);
